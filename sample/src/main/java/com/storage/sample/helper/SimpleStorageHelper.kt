...
class SimpleStorageHelper {
    ...
    @JvmOverloads
    constructor(activity: ComponentActivity, savedState: Bundle? = null, permissions: Array<String>) {
        storage = SimpleStorage(activity)
        init(savedState)
        permissionRequest = ActivityPermissionRequest.Builder(activity)
    ...
    @JvmOverloads
    fun setCameraPermission(activity: ComponentActivity, savedState: Bundle? = null) {
        storage = SimpleStorage(activity)
        init(savedState)
        permissionRequest = ActivityPermissionRequest.Builder(activity)
            .withPermissions(*rwPermission)
    ...
    private val permissionCallback: PermissionCallback
        get() = object : PermissionCallback {
            override fun onPermissionsChecked(result: PermissionResult, fromSystemDialog: Boolean) {
                ...
                if (!granted) {
                    Toast.makeText(storage.context,
                        R.string.ss_please_grant_storage_permission, Toast.LENGTH_SHORT).show()
                }
    ...
    @JvmOverloads
    fun captureImage(requestCode: Int = storage.requestCodeFolderPicker) {
        pickerToOpenOnceGranted = TYPE_FOLDER_PICKER
        originalRequestCode = requestCode
        storage.captureImage(requestCode)
    }
